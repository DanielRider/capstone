...
class DP():
    def __init__(self,df,target,fillna_mean=[],fillna_median=[],fillna_0=[],drop=[],DROP_PCTNULL=0.9,TOP_BIN_FLAGS=5,save_to = None,filename = None):
    ...
    def process(self):
        ...
        for i in binaryFlags:
            try:    
                ...
                if self.df[i].value_counts().count() > self.TOP_BIN_FLAGS:
                    top_vals=self.df.sort_values(by=i,ascending=False).groupby(i).count().reset_index()[:(self.TOP_BIN_FLAGS-1)][i]
